...
/**
3个重载的drawImage方法，具体使用要点在下面小节中演示
drawImage(image : HTMLImageElement | HTMLCanvasElement | HTMLVideoElement |
    ImageBitmap, dstX : number, dstY : number) : void ;
drawImage(image : HTMLImageElement | HTMLCanvasElement | HTMLVideoElement |
    ImageBitmap, dstX : number, dstY : number, dstW : number, dstH :
    number) : void ;
drawImage(image : HTMLImageElement | HTMLCanvasElement | HTMLVideoElement |
    ImageBitmap, srcX : number, srcY : number, srcW : number, srcH :
    number, dstX : number, dstY : number, dstW : number, dstH :
    number) : void ;
// 创建ImageData对象
createImageData(imageDataOrSw : number | ImageData, sh ? : number) :
ImageData;
// 从CanvasRenderingContext2D渲染上下文对象中获取ImageData对象
getImageData(sx : number, sy : number, sw : number, sh : number) :
ImageData;
// 设置CanvasRenderingContext2D渲染上下文对象中部分或全部像素数据
putImageData(imagedata : ImageData, dx : number, dy : number, dirtyX ? :
    number, dirtyY ? : number, dirtyWidth ? : number, dirtyHeight ? :
    number) : void ;
...
export default class ImageTest extends Canvas2DApplication {
    // 参数是要载入的image的URL路径
    public loadAndDrawImage(url: string, descRect?: Rectangle, fillStyle: EImageFillType = EImageFillType.STRETCH): void {
    ...
    public drawColorCanvas(): void {
        ...
        this.drawImage(colorCanvas, Rectangle.create(100, 100,
    ...
    // 第1列的黑色子矩形反转为白色
    public testChangePartCanvasImageData(rRow: number = 2, rColum: number =
        0, cRow: number = 1, cColum: number = 0, size: number = 32): void {
        // 调用getColorCanvas方法生成16种标准色块离屏画布
        let colorCanvas: HTMLCanvasElement = this.getColorCanvas(size);
        // 获取离屏画布的上下文渲染对象
        let context: CanvasRenderingContext2D | null = colorCanvas.getContext
            ("2d");
        if (context === null) {
        ...
        // 每个像素又有4个分量[ r , g , b , a ]
        let imgData: globalThis.ImageData = context.createImageData(size, size);
        ...
        context.putImageData(imgData, size * rColum, size * rRow, 0,
            0, size, size);
        // 显示未修改时的离屏画布的效果
        this.drawImage(colorCanvas, Rectangle.create(100, 100,
            colorCanvas.width, colorCanvas.height));
        ...
        this.drawImage(colorCanvas,
            Rectangle.create(300, 100, colorCanvas.width, colorCanvas.
                height));
    }
...
_createtab(
    '绘制图片 - 实现repeat',
    ["原图绘制", "拉伸绘制", "repeatX绘制", "repeatY绘制", "repeat绘制", "离屏渲染", "理解ImageData"],
    {
        onChange(k) {
            app.clearRect();
