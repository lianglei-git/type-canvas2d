...
class Doom3Token {
    ...
    isString(str) {
        let count = this._chartArr.length;
        if (str.length !== count) {
            return false;
        }
...
export class Doom3Tokenizer {
    // 增加一个私有变量_current，并使用new进行初始化接口
    _current = new Doom3Token();
...
let token = tokenizer.createDoom3Token();
