...
import { Math2D, ELayout } from '../../Canvas2D/core/index';
export default class Coord extends GeometryTes {
    ...
    drawCanvasCoordCenter() {
        ...
        // 计算出Canvas的中心点坐标
        let halfWidth = this.canvas.width * 0.5;
        let halfHeight = this.canvas.height * 0.5;
        this.context2D.save();
    ...
    //     }
    // }
    doTransform(degree, rotateFirst = true) {
        if (this.context2D == null)
            return;
        // 要绘制的矩形的尺寸
        let width = 100;
    ...
    doTransformStrokeCircle1() {
        // 调用前面实现的两点间距离公式
        // 第一个点是原点，第二个点是画布中心点
        let radius = this.distance(0, 0, this.canvas.width * 0.5, this.canvas.height * 0.5);
        // 然后绘制一个圆
        this.strokeCircle(0, 0, radius, 'black');
    }
    /** 旋转并绘制轨迹 2 */
    doTransformStrokeCircle2(degree = 20, rotateFirst = true) {
        if (this.context2D == null)
            return;
        this.drawCanvasCoordCenter();
        this.strokeGrid();
    ...
    rotateTranslate(degree, layout = ELayout.LEFT_TOP, width = 40, height = 20) {
        if (this.context2D === null) {
            return;
        }
        // 将角度转换为弧度，由此可见，本方法的参数degree是以角度而不是弧度表示
    ...
    testFillLocalRectWithTitle() {
        if (this.context2D !== null) {
            ...
            // 计算半径
            let radius = this.distance(0, 0, this.canvas.width
    ...
    doLocalTransform() {
        ...
        let width = 100; // 在局部坐标系中显示的rect的width
        let height = 60; // 在局部坐标系中显示的rect的height
        let coordWidth = width * 1.2; // 局部坐标系x轴的长度
        let coordHeight = height * 1.2; // 局部坐标系y轴的长度
        let radius = 5; // 绘制原点时使用的半径
        this.context2D.save();
        /*
        所有局部坐标系变换演示的代码都现在此处
        本节下面的所有绘制代码都写在此处
    ...
    translateRotateTranslateDrawRect(degree, u = 0, v = 0, radius = 200, width = 40, height = 20) {
        if (this.context2D === null) {
            return;
        }
        // 将角度变换为弧度
    ...
    testFillLocalRectWithTitleUV() {
        if (this.context2D === null) {
            return;
        }
        let radius = 200; // 圆路径的半径为200个单位
        let steps = 18;
...
canvas.width = 500;
canvas.height = 500;
_createTab("局部坐标系变换", ['drawCanvasCoordCenter', '位置信息', '先平移再旋转', '先旋转再平移', '旋转轨迹1', '旋转轨迹2', '旋转轨迹3', '变换过程1', '变换过程2'], {
    onChange(k) {
        app.stop();
        app.clearRect();
        switch (k) {
            case 'drawCanvasCoordCenter':
                ...
                break;
            case '位置信息':
                window.Spui.Message.info('鼠标在画布移动试试');
                app.start();
                break;
            case '先平移再旋转':
                app.doTransform(20);
