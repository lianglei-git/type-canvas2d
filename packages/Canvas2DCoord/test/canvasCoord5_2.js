...
class Tank {
    ...
    scaleY = 1.0;
    // 坦克当前的旋转角度
    tankRotation = 0; //整个坦克的旋转角度，弧度表示
    turretRotation = 0; //炮塔的旋转角度，弧度表示
    // 在Tank类中增加一个成员变量，用来标示Tank初始化时是否朝着y轴正方向
    initYAxis = true;
    showLine = true;
    //是否显示坦克原点与画布中心点和目标点之间的连线
    showCoord = false; //是否显示坦克本身的局部坐标系
...
class Application5_2 extends Coord {
    ...
    drawTriangle(x0, y0, x1, y1, x2, y2, stroke = true) {
        if (this.context2D === null) {
            return;
        }
        this.context2D.save();
...
let len = app.distance(ptX, ptY, app.canvas.width * 0.5, app.canvas.height * 0.5);
